                    ...
                    <ul class="flex flex-col m-3">
                        <li class="flex flex-row">
                            <div
                                ...
                                @else
                                    <img class="rounded-full w-12 h-12"
                                        src="./img/{{ $friend->avatar }}" alt="avatar friend">
                                @endif
                                <h1 class="text-gray-600">{{ $friend->name }}</h1>
                                <form wire:submit.prevent='acceptRequest({{ $friend->id }})'>
                                    <button type="submit"
                                        class="px-2 py-1 rounded-xl bg-green-300 text-green-600 max-w-max shadow-sm hover:shadow-lg">Accept</button>
                                </form>
                                <form wire:submit.prevent='denyRequest({{ $friend->id }})'>
                                    <button type="submit"
                                        class="px-2 py-1 rounded-xl bg-red-300 text-red-600 max-w-max shadow-sm hover:shadow-lg">Deny</button>
                                </form>
                            </div>
                        </li>
                    </ul>
